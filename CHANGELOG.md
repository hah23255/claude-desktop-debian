...
- Adopted the Engineering Doctrine Playbook as the repository golden standard (`CLAUDE.md`, `AGENTS.md`): every rule cites exactly one enforcer, CI gates are fail-closed, and the Master Tester Doctrine (witness test first, observable-outcome naming, evidence-pinned commits, tooling tests its own output shape) governs any fix that closes a finding.
