...
- Cowork VM-download guard (patch C1 in cowork-bwrap) adapted for upstream 1.46388.2. Upstream refactored the download function (from QH to YU, introducing an `await wB()` prelude ahead of the status check), causing C1 anchor matching to fail. Restored `patch_cowork_bwrap` in active patches, updated C1 anchor regex to match both the legacy and 1.46388.2 downloadVM function signatures with fail-closed negative control tests, and adopted the Engineering Doctrine Playbook as the repository golden standard.
